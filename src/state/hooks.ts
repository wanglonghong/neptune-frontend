...
import { Toast, toastTypes } from '@neptuneswap/neptuneswapuikit'
...
export const usePriceBnbBusd = (): BigNumber => {
  const pid = 1 // TUNE-BNB LP
  const neptunePriceUSD = usePriceCombustBusd();
  const farm = useFarmFromPid(pid)
  return farm.tokenPriceVsQuote ? neptunePriceUSD.div(farm.tokenPriceVsQuote) : ZERO
}

export const usePriceCombustBusd = (): BigNumber => {
  const pid = 2 // TUNE-BUSD LP
  const farm = useFarmFromPid(pid)
